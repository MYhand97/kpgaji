        ...
        <div class="container">
            <div class="card card-signin">
                <div class="card-body">
                    ...
                    if(isset($_GET['id'])){
                        $id=$_GET['id'];
                        $select = mysqli_query($conn, "select * from jabatan where id='$id'") or die(mysqli_error($conn));
                    ...
                    if(isset($_POST['submit'])){
                        $jbtan = $_POST['jbtan'];
                        $bgian = $_POST['bgian'];
                        $query=mysqli_query($conn, "update jabatan set jbtan='$jbtan', bagian='$bgian' where id='$id'") or die(mysqli_error($conn));
                        if($query){
                            echo '<script>alert("Berhasil mengedit data."); document.location="../data_jabatan.php";</script>';
                        }
                        else{
                            echo '<div class="alert alert-warning">Gagal Edit Jabatan!</div>';
                        }
                    }
                    ?>

                    <br>
                    <form class="form-signin" action="edit_jabatan.php?id=<?php echo $id; ?>" method="POST">
                        <div class="form-label-group">
                            <input type="text" id="jbtan" name="jbtan" class="form-control" placeholder="Jabatan" value="<?php echo $data['jbtan']; ?>" required autofocus>
                        </div>
                        <br>

                        <div class="form-label-group">
                            <input type="text" id="bgian" name="bgian" class="form-control" placeholder="Bagian" value="<?php echo $data['bagian']; ?>" required>
                        </div>
                        <br>
