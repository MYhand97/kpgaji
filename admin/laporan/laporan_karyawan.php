...
<body>
            ...
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                        <a class="dropdown-item active" href="laporan_karyawan.php">Laporan Data Karyawan</a>
                        <a class="dropdown-item" href="laporan_absen.php">Laporan Absen Karyawan</a>
                        <a class="dropdown-item" href="laporan_gaji.php">Laporan Gaji Karyawan</a>
                    </div>
                </li>
            </ul>
    ...
    <main role="main">
        ...
        <div class="jumbotron">
            <div class="container">
                <br><br>
                <h1 class="display-3">Laporan Data Karyawan</h1>
            </div>
        </div>
        ...
        <div class="form-inline d-flex justify-content-center md-form form-sm mt-0">
            ...
            <a class="btn btn-success ml-3" href="buat_laporan_karyawan.php" role="button">Export Excel</a>
            <a class="btn btn-success ml-3" href="upload_laporan_karyawan.php" role="button">Upload</a>
        </div>
        ...
        <div class="container-fluid table-responsive-xl">
            ...
            <table class="table table-hover">
                ...
                <tbody id="myTable">
                    <?php
                    $query = mysqli_query($conn, "select * from filekaryawan order by nfile asc");
                    if(mysqli_num_rows($query) > 0){
                        $no = 1;
                        while($row = mysqli_fetch_array($query)){
                            echo '<tr>';
                            echo '<th scope="row">'.$no.'</th>';
                            echo '<td>'.$row['tglupload'].'</td>';
                            echo '<td>'.$row['nfile'].'</td>';
                            echo '<td>'.$row['tipefile'].'</td>';
                            echo '<td>'.formatSizeUnits($row['ukfile']).'</td>';
                            echo '<td>
                            <a class="btn btn-success btn-sm" href="'.$row['file'].'" role="button">Download</a>
                            <a class="btn btn-danger btn-sm" href="hapus_laporan_karyawan.php?id='.$row['id'].'&file='.$row['file'].'" role="button" onclick="return confirm(\'yakin ingin menghapus file ini?\')">Hapus</a>
                        ...
                        echo '
                        <tr>
                            <td scope="row" align="center" colspan="6" class="bg-danger">Tidak ada data!</td>
                        </tr>
                        ';
                    }
                    ?>
